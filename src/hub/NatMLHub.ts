 ...
 *
 */
export abstract class NatMLHub {
  // Prevent instantiation with `abstract` since no `static` classes in TS
  ...
  public static get URL() {
    switch (process.env.HUB_ENV?.toLowerCase()) {
      ...
      case "staging":
        return "https://staging.api.natml.ai/graph";
      default:
        return "https://api.natml.ai/graph";
    }
  }
  ...
  public static async createSession(
    ...
    else return response.data.data.createSession;
  }
  ...
  public static async requestPrediction<T extends boolean>(
    ...
    if (response.data.errors)
      throw new Error(JSON.stringify(response.data.errors));
    else return response.data.data.requestPrediction;
  }
   ...
   */
  public static async uploadURL(
    type: UploadType = UploadType.Feature,
    name?: string
  ): Promise<string> {
    const query = `
    ...
    if (response.data.errors)
      throw new Error(JSON.stringify(response.data.errors));
    else return response.data.data.uploadURL;
  }
  //#endregion
