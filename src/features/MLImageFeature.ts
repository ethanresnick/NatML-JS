...
export class MLImageFeature extends MLFeature implements IMLHubFeature {
     ...
     */
    public constructor (input: Buffer | string | Uint8Array, attributes?: PixelBufferAttributes) {
        let type: MLImageType;
        if (attributes)
            type = new MLImageType(MLDataType.Byte, [1, attributes.height, attributes.width, attributes.channels]);
        else
