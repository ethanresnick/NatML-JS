  ...
  private static deserialize(input: MLHubFeature): MLFeature {
    switch (input.type) {
      case MLDataType.Float:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Double:
        return MLHubModel.deserializeArray(input);
      case MLDataType.SByte:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Short:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Int:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Long:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Byte:
        return MLHubModel.deserializeArray(input);
      case MLDataType.UShort:
        return MLHubModel.deserializeArray(input);
      case MLDataType.UInt:
        return MLHubModel.deserializeArray(input);
      case MLDataType.ULong:
        return MLHubModel.deserializeArray(input);
      case MLDataType.Image:
        return MLHubModel.deserializeImage(input);
      case MLDataType.Audio:
        return MLHubModel.deserializeAudio(input);
      case MLDataType.String:
        return MLHubModel.deserializeText(input);
      default:
        throw new Error(
          `Cannot deserialize invalid feature type: ${input.type}`
        );
    }
  }
    ...
    switch (input.type) {
      ...
      case MLDataType.Long:
        typedArray = new BigInt64Array(
          buffer,
          offset,
          length / BigInt64Array.BYTES_PER_ELEMENT
        ) as any;
        break;
      case MLDataType.Byte:
      ...
      case MLDataType.ULong:
        typedArray = new BigUint64Array(
          buffer,
          offset,
          length / BigUint64Array.BYTES_PER_ELEMENT
        ) as any;
        break;
      default:
        throw new Error();
